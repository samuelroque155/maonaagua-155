...
export default function App() {
  ...
  // NOVO ESTADO: Controle da Fila de Fechamento do Mês
  ...
  if (tela === 'relatorio') {
    // --- NOVO: TELA DA FILA DE ENVIO MANUAL ASSISTIDO ---
    if (modoFilaEnvio) {
      // Filtra apenas os clientes que tiveram alguma manutenção registrada no mês atual
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 p-4 text-zinc-900 dark:text-zinc-100 max-w-md mx-auto font-sans transition-colors duration-300 pb-20">
          ...
          <div className="bg-sky-50 dark:bg-sky-900/20 p-5 rounded-[1.25rem] border border-sky-200 dark:border-sky-800 mb-6 shadow-sm">
            <p className="text-xs text-sky-800 dark:text-sky-300 font-medium leading-relaxed">
              ...
              <b>Passo 1:</b> Abra a pasta e salve o PDF no celular.<br/>
              <b>Passo 2:</b> Clique em "Enviar Zap", anexe o PDF e mande!
            </p>
          </div>
          ...
          <div className="space-y-4">
            {clientesComVisitaEsteMes.length === 0 ? (
              ...
              <div key={c.id} className="w-full bg-white dark:bg-zinc-900 p-5 rounded-[1.5rem] border border-zinc-200 dark:border-zinc-800 flex flex-col gap-4 shadow-sm">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setClienteRelatorio(c); setModoFilaEnvio(false); setTela('ver_relatorio'); }} className="flex-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 text-xs hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
                    <FileText size={16} /> 1. Salvar PDF
                  </button>
                  <button onClick={() => {
                  ...
                  }} className="flex-1 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 text-xs transition-colors shadow-md">
                    <Send size={16} /> 2. Enviar Zap
                  </button>
                </div>
              </div>
            ))}
